...
Because the p-value > $\alpha$ = 0.05, we fail to reject H0 and conclude that there is no evidence against Poisson
...
$$y \pm z_{0.025} * sd / \sqrt{n}$$
```{r}
y = mean(poisson_data$Y) 
s = sd(poisson_data$Y)

CI_low <- (y - (1.96*s/sqrt(50)))

CI_high <- (y + (1.96*s/sqrt(50)))
```
